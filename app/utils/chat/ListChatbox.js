...
class ListChatBox extends React.Component {
            ...
            return (
              <div key={index}>
                ...
                <span className="item-user">
                  <div
                    onClick={self.reOpenchatbox.bind(this, index, user)}
                    className="name-user-behide"
                    style={{ cursor: "point" }}
                  >
                    {user.fullname}
                  </div>{" "}
                  <i
                    onClick={self.remove.bind(this, index, user)}
  ...
  exitsChatbox(user) {
    ...
    for (var i = 0; i < listUser.length; i++) {
      if (listUser[i].id == user.id) return i;
      break;
    }
    return -1;
  }
    ...
    io.socket.get("/notification/chat", function gotResponse(data, jwRes) {
      ...
      io.socket.on("chatuser" + data.userId, function(data) {
        ...
        if (index != -1) {
          ...
          self.props.dispatch(
            addMessage({ message: data.data, id, new_message: data })
          );
        } else {
  ...
  render() {
    ...
    let behidechat =
      listChatbox.length > 3 ? (
        <div className="chat-window-hide">
          <OverlayTrigger
            placement="top"
            trigger="click"
            overlay={this.popoverFocus()}
          >
            <Button>
              {" "}
      ...
      <div>
        {listChatbox.length && listChatbox.length > 0
          ? listChatbox.map((chatbox, index) => {
              let id =
                auth.user.id > chatbox.id
                  ? auth.user.id + "_" + chatbox.id
                  : chatbox.id + "_" + auth.user.id;
              if (index < 3)
                return (
                  <ChatBox
                    key={index}
                    ...
                    dispatch={dispatch}
                    auth={auth}
                    listchat={this.props.chatbox.listchat[id]}
                    right={(index + 1) * 243}
                    key={index}
                    chatbox={chatbox}
                  />
