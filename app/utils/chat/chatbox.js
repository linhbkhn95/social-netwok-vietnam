var HeaderChatBox = React.createClass({
  ...
  render() {
    return React.DOM.div(
      ...
      React.DOM.div(
        { className: "col-md-8 col-xs-8 head-name-chatbox" },

        React.DOM.h3(
          { className: "panel-title name-chatbox" },
          React.DOM.a(
      ...
      React.DOM.div(
        { className: " head-icon-close-open-chatbox" },
        React.DOM.a(
          null,
          React.DOM.span({
            id: "minim_chat_window",
            className: "glyphicon glyphicon-minus icon_minim icon-close-open"
          })
        ),
        React.DOM.a(
          { onClick: this.closeChatBox },
          React.DOM.span({
            className: "glyphicon glyphicon-remove icon-close-open icon_close",
            id: "chat_window_1"
          })
        )
      )
...
var FooterChatBox = React.createClass({
  ...
  getTimeMsg() {
    var objToday = new Date(),
      ...
      domEnder = (function() {
        var a = objToday;
        if (/1/.test(parseInt((a + "").charAt(0)))) return "ngày";
        a = parseInt((a + "").charAt(1));
        return 1 == a ? "ngày " : 2 == a ? "ngày" : 3 == a ? "ngày" : "ngày";
      })(),
      dayOfMonth =
        domEnder + today + (domEnder + objToday.getDate() < 10)
          ? "0" + objToday.getDate()
          : objToday.getDate(),
      months = new Array(
        " 1",
...
var ChatBox_Demo = React.createClass({
  ...
  componentWillMount: function() {
    ...
    this.chatBoxSub = channel.subscribe(
      "ChatBox" + this.props.username + ".Add",
      this.chatBoxAdd
    );
  },
  ...
  chatBoxAdd: function(data) {
    console.log(data);
    this.props.messages.sync("create", {
      message: data.text,
      user_recevice: data.username
    });
  },
    ...
    return React.DOM.div(
      ...
      this.state.names.map((name, index) => {
        return ChatBox_Demo({
          ...
          right: this.state.listStyle[index],
          key: index
        });
      })
    );
...
var TabChatBox = React.createClass({
  ...
  render() {
    ...
    return React.DOM.div(
      ...
      ListChatBox({
        me: this.props.me,
        onMessageSubmit: this.onMessageSubmitChatBox,
        closeChatBox: this.closeChatBox,
        names: this.state.listchat
      })
    );
  }
});
