...
class ChatBox extends React.Component {
  ...
  remove() {
    ...
    let id =
      me.id > chatbox.id ? me.id + "_" + chatbox.id : chatbox.id + "_" + me.id;
    this.props.dispatch(removeChatbox({ index, id }));
  }
  onFocusInput() {
    io.socket.post(
      "/chat/updateStatusListMessage",
      { userId_patner: this.props.chatbox.id },
      resdata => {
        if (resdata.EC == 0) {
      ...
      >
        <div className="col-xs-12 col-md-12">
          <div className="panel panel-default">
            ...
            <div
              id="bodychat"
              className="panel-body col-md-12 msg_container_base"
            >
              ...
              {listMessage.length > 0
                ? listMessage.map((message, index) => {
                    ...
                    if (
                      index > 1 &&
                      message.userId_sent != user.id &&
                      listMessage[index].userId_sent ==
                        listMessage[index - 1].userId_sent
                    ) {
                      (show = false),
                    ...
                    if (message.userId_sent == user.id)
                      return (
                        <div key={index}>
                          ...
                          <div className=" message-chatbox-sent">
                            {" "}
                      ...
                      return (
                        ...
                        {show ?   <div className="img-chatbox">
                            ...
                            {show ? (
                              <NavLink
                                to={"/userpage" + self.props.chatbox.username}
                              >
                                <img
                                  style={{ marginRight: "3px" }}
                                  className="img-user"
                                  src={self.props.chatbox.url_avatar}
                                />
                              </NavLink>
                            ) : (
                          ...
                          <div style={style} className="content-text">
                            {" "}
                            <div className=" message-chatbox-rc">
                              <div>{message.text}</div>
                            </div>
                          </div>
                        </div>
