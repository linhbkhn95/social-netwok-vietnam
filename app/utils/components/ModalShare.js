...
class ModalShare extends React.Component {
  ...
  onChange(type, event) {
    if (event && event.target) this.state[type] = event.target.value;
    else {
  ...
  post(files) {
    ...
    FileUpload.upload(files).then(response => {
      if (response.data.EC == 0) {
        io.socket.post(
          "/post/postStatus",
          { data, urls_file: response.data.DT },
  ...
  render() {
    return (
      <Modal {...this.props} aria-labelledby="contained-modal-title-lg">
          ...
          <Modal.Title id="contained-modal-title-lg">
            <div className="title-order">
              <div className="title-modal-post">
                Chia sẻ trên dòng thời gian của bạn
              </div>
            </div>
          </Modal.Title>
        </Modal.Header>
        ...
        <Modal.Footer>
          <Button bsStyle="success" onClick={this.share.bind(this)}>
            Đăng
          </Button>
