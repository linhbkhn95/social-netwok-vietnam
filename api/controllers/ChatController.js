...
module.exports = {
	add:function(req,res){
        ...
        Chat.create(body).exec(async(err,chat)=>{
            ...
            if(chat){
                ...
                let key_chat = chat.userId_sent>chat.userId_rec?chat.userId_sent+'_'+chat.userId_rec:chat.userId_rec+'_'+chat.userId_sent
                Chat_new_user.findOne({key_chat}).exec(async(err,chat_new_user)=>{
                    if(chat_new_user){
                        let chatnew = await Chat_new_user.update({key_chat},{
                            time :moment().format(),
    ...
    updateKnowMessage:function(req,res){
        ...
        Chat_new_user.update({key_chat:id},{know_message:true}).exec(async(err,chat_new_user)=>{
            if(err){

            }
           let message = await Chat.update({id:chat_new_user.id_message_new},{read_message:true});
    ...
    updateStatusListMessage:function(req,res){
        let {userId_patner} = req.body
        Chat.update({userId_rec:req.session.user.id,read_message:false},{read_message:true}).exec((err,list)=>{
            if(err){
    ...
    getlist_message_user:function(req,res){
        let userId = req.session.user.id
            Chat.query("call webandanh.getlist_message_user(?)", [userId], function (err, [data, server_status]) {
                if (err) {
                    return res.send(OutputInterface.errServer('Lỗi hệ thống'))
                }
                Promise.all(data.map((item)=>{
                    return new Promise(async(resolve)=>{
                         let userId_patner  = item.userId_sent==userId?item.userId_rec:item.userId_sent
                    ...
                    Chat.query("call webandanh.get_count_wait_user(?)", [userId], function (err, [count, server_status]) {
                        let data = {
