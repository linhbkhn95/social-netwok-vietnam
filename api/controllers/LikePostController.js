...
module.exports = {
  accessLike: async function(req, res) {
    ...
    Likepost.findOne({ postId, userId }).exec(async (err, likePost) => {
      ...
      if (likePost) {
        if (likePost.status) post.countLike -= 1;
        else post.countLike += 1;
        await post.save({});

        likePost.status = !likePost.status;
        likePost.save(function() {
          if (likePost.status) NotificationUtils.notifiPostUser_Like(post, req);

          sails.sockets.broadcast(
            "Subscribe_Status",
            postId + TypeSocket.like,
            UtilsSocket.getData(
              userLike,
              TypeSocket.like,
        ...
        post.countLike += 1;
        await post.save({});
        Likepost.create({ postId, userId, status: true }).exec(
          (err, result) => {
            sails.sockets.broadcast(
              "Subscribe_Status",
              postId + TypeSocket.like,
              UtilsSocket.getData(userLike, TypeSocket.like, VerbSocket.like),
              req
            );
            NotificationUtils.notifiPostUser_Like(post, req);
            return res.send(
              OutputInterface.success({ countLike: post.countLike, like: 1 })
            );
          }
        );
      }
  ...
  getlist_LikeFormatPost: function(req, res) {
    ...
    Likepost.find({ postId, status }).exec((err, list) => {
      ...
      Promise.all(
        ...
        return res.send(
          OutputInterface.success({ listUserId: response, listUser })
        );
      });
    });
  }
