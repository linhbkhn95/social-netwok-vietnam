...
module.exports = {
        accessFollow:async function(req,res){
            ...
            Follows.findOne({userId_follow:req.session.user.id,userId:userPatner.id}).exec((err,follow)=>{
                if(err){
                ...
                if(follow){
                    ...
                    Follows.create({userId_follow:req.session.user.id,userId:userPatner.id,status:1}).exec((err,followuser)=>{
                        if(err){
