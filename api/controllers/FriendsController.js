...
module.exports = {
  getlistRequestFriend: function(req, res) {
    Friends.find({
      ...
      .where({ action_userId: { "!": [req.session.user.id] } })
      .exec((err, list) => {
        Promise.all(
          list.map(item => {
            return new Promise(async (resolve, reject) => {
              let userPatnerId =
                item.userId_one == req.session.user.id
                  ? item.userId_two
                  : item.userId_one;

              let userPatner = await User.findOne({
                id: userPatnerId,
                select: ["fullname", "url_avatar", "username"]
              });
              resolve(userPatner);
            });
          })
  ...
  accept: async function(req, res) {
    ...
    let userId_one =
      req.session.user.id < userPatner.id ? req.session.user.id : userPatner.id;
    let userId_two =
      req.session.user.id > userPatner.id ? req.session.user.id : userPatner.id;
  ...
  accessFriend: function(req, res) {
    ...
    User.findOne({ username }).exec(async (err, user) => {
      ...
      if (user) {
        ...
        if (friend) {
          ...
          sails.sockets.broadcast(
            "NotificationUser",
            "notifi_user_requestFriend" + user.id,
            dataSocket,
            req
          );
          ...
          Friends.create({
            ...
            userId_two,
            status: 0,
            action_userId: req.session.user.id
          }).exec((err, result) => {
            if (err) {
              return res.send(OutputInterface.errServer(err));
            }
      ...
      } else return res.send(OutputInterface.errServer("no user"));
    });
  },
  getCanFriend: async function(req, res) {
    let userId = req.session.user.id;
    Friends.find({
      or: [{ userId_one: userId, status: 1 }, { userId_two: userId, status: 1 }]
    }).exec(async (err, list) => {
      if (err) {
      }
      let listUserId = list.map(friend => {
        return friend.userId_one == userId
          ? friend.userId_two
          : friend.userId_one;
      });
      let listUser;
  ...
  checkFriend: async function(req, res) {
    ...
    if (userPatner) {
      let userId_one =
        req.session.user.id < userPatner.id
          ? req.session.user.id
          : userPatner.id;
      let userId_two =
        req.session.user.id > userPatner.id
    ...
    } else return res.send(OutputInterface.errServer(""));
  },
